...
const CrashGame = ({ onBack }) => {
  ...
  const [airplanePosition, setAirplanePosition] = useState({ x: 5, y: 100 });
    ...
    if (gameState === 'playing') {
      interval = setInterval(() => {
        setMultiplier(prev => {
          ...
          // Update flight path - curved flight from top to bottom like real Aviator
          const progress = Math.min((newMultiplier - 1) / 8, 1); // Progress over first 9x
          const xPos = 5 + progress * 85; // Start from 5%, move to 90%
          const yPos = 100 + Math.pow(progress, 0.6) * 220; // Start from top, curve down
  ...
  useEffect(() => {
    ...
    if (flightPath.length > 1) {
      // Scale coordinates for canvas - adjusted for top-to-bottom flight
      const scaledPath = flightPath.map(point => ({
        x: (point.x / 100) * width,
        y: ((point.y - 100) / 200) * height
      }));
          ...
          <div className="flight-overlay">
            ...
            <div 
              ...
              style={{
                ...
                top: `${Math.min((airplanePosition.y - 100) / 200 * 100, 90)}%`,
